...
class ParticleFilter(object):
    ...
    def predict(self, odom):
        ...
        if odom[0]==0 and odom[1]==0 and odom[2]==0:
            ...
            # Partical positions with Gaussian noise
            '''
            Robot only goes in its x direction. What we need to do is to transform 
            the small changes between two poses
            from the robot coordinate to the world coordinate, then we can use it as the position of particles
            '''
    ...
    def weight(self, lines):
        ...
        # Get the length of each line in the map and measured lines
        # In order to compare them in the optional part
